...
local ViMode = {
  ...
  provider = function(self) return " %8(" .. self.mode_names[self.mode] .. "%)" end,
...
local FileName = {
  init = function(self)
    ...
    if vim.bo and vim.bo.filetype == "help" then
      ...
      self.icon, self.icon_color = icons.Help, "mauve"
    else
  ...
  end,
  {
    ...
    {
      provider = function(self) return self.icon .. " " .. self.filename end,
    },
    {
...
local Diagnostics = {
  ...
  init = function(self)
    self.show = function(severity) return self[severity] > 0 and (self.signs[severity] .. " " .. self[severity]) end
...
local Git = {
  condition = function() return require("heirline.conditions").is_git_repo and vim.b.gitsigns_status_dict end,
  init = function(self)
    self.status_dict = vim.b.gitsigns_status_dict
    self.has_changes = self.status_dict and self.status_dict.added ~= 0
      or self.status_dict.removed ~= 0
      or self.status_dict.changed ~= 0
  end,
  {
    { -- branch name
      provider = function(self) return icons.GitBranch .. " " .. self.status_dict.head end,
      hl = { fg = "magenta" },
    },
    {
      {
        ...
        provider = function(self)
          ...
          return count > 0 and (" " .. icons.GitAdd .. " " .. self.status_dict.added)
        end,
        hl = { fg = "green" },
      },
      {
        ...
        provider = function(self)
          ...
          return count > 0 and (" " .. icons.GitDelete .. " " .. self.status_dict.removed)
        end,
        hl = { fg = "red" },
      },
      {
        ...
        provider = function(self)
          ...
          return count > 0 and (" " .. icons.GitChange .. " " .. self.status_dict.changed)
        end,
        hl = { fg = "red" },
      },
    },
...
local MacroRec = {
  ...
  provider = function(self) return icons.MacroRecording .. " " .. self.reg end,
...
local TablineFileFlags = {
  { -- modified
    ...
    provider = function() return icons.FileModified .. " " end,
    hl = { fg = "green" },
  },
  { -- read-only
    condition = function(self) return vim.api.nvim_buf_get_option(self.bufnr, "readonly") end,
    provider = function() return icons.FileReadOnly .. " " end,
    hl = { fg = "red" },
  },
}

local FileActive = {
  provider = function(self) return self.is_active and icons.GitSign or " " end,
  hl = { fg = "blue", bg = "bg" },
}
...
local TablineExplorer = {
  ...
  provider = function(self)
    ...
    if width - 1 % 2 then lb = lb .. " " end
    return lb .. icons.Tree .. rb
  end,
}

local TablineTabNumber = {
  init = function(self) self.tabnr = vim.api.nvim_tabpage_get_number(0) end,
  provider = function(self) return " " .. icons.Tab .. " " .. self.tabnr .. " " end,
...
local has_prefix = function(str, prefix) return str:sub(1, #prefix) == prefix end
...
local FileNameWinbar = {
  ...
  init = function(self)
    ...
    elseif self.plugins[vim.bo.filetype] then
      ...
      self.icon, self.icon_color = icons.Plugin, "mauve"
    else
      self.icon, self.icon_color =
        require("nvim-web-devicons").get_icon_color_by_filetype(vim.bo.filetype, { default = true })
    end
...
return {
  ...
  event = "BufEnter",
  dependencies = { "catppuccin/nvim", "nvim-tree/nvim-web-devicons", "lewis6991/gitsigns.nvim", "SmiteshP/nvim-navic" },
  opts = function()
