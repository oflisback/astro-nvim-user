...
function M.calc_todo_indent_level(lines_above_plus_current)
  ...
  if stars_on_line > 0 then
    ...
    print "current line has stars, respecting it"
  ...
  for index = #lines_above_plus_current - 1, 1, -1 do
    ...
    if stars_on_line > 0 then
      if is_todo then
        print "inspected above line is todo, returning its number of stars"
        return stars_on_line
      else
        -- We found a header above the current line, the todo level should
        -- be that level plus one
        print "inspected above line is header, returning its number of stars plus one"
...
function M.apply_todo_transformation()
  ...
  if indent_level == 0 then
    ...
    local top_line = math.max(1, current_line_nbr - 1)
    local lines = vim.api.nvim_buf_get_lines(bufnr, top_line, current_line_nbr + 1, false)
